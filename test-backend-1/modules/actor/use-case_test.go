...
import (
	...
	mocks_repositories "test-backend-1/repositories/mocks"
	mocks_db "test-backend-1/utils/db/mocks"
	"testing"
)
...
func Test_useCase_ActivateActor(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:         args{username: "randomUser", actor: entities.Actor{}},
			wantErr:      false,
			activeStatus: true,
		},
		{
			...
			fields:       defaultUseCase(t),
			args:         args{username: "otherRandom", actor: entities.Actor{}},
			wantErr:      true,
			activeStatus: false,
		},
	}
